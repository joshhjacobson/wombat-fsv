...
read_residual <- function(filename) {
  gridded <- read_gridded_data(
    filename,
    'residual'
  )
...
compute_posterior <- function(prior, design_matrix, samples_df, posterior_name) {
  prior %>%
    mutate(
...
add_samples_alpha_delta <- function(samples_alpha, samples_delta) {
  stopifnot(
    all.equal(
      levels(samples_alpha$alpha_df$basis_vector),
      levels(samples_delta$delta_df$basis_vector)
    )
  )
  n_samples <- samples_delta$n_samples
  n_levels <- nlevels(samples_alpha$alpha_df$basis_vector)
  alpha_basis <- as.integer(samples_alpha$alpha_df$basis_vector)
  delta_basis <- as.integer(samples_delta$delta_df$basis_vector)

  alpha_samples <- tail(samples_alpha$alpha, samples_alpha$n_samples - samples_alpha$n_warm_up)

  delta_samples_padded <- matrix(0, nrow = n_samples, ncol = n_levels)
  delta_samples_padded[, delta_basis] <- samples_delta$delta_scaled
  samples_added <- alpha_samples + delta_samples_padded[, alpha_basis]

  list(
    samples = samples_added,
    samples_df = samples_alpha$alpha_df %>%
      select(-c(value, value_samples)) %>%
      mutate(
        value = colMeans(samples_added),
        value_samples = t(samples_added)
      )
  )
}
